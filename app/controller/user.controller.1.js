  ...
  if (req.body && req.body.firstname && req.body.email && req.body.password) {
    ...
    db.query(sql, (err, result) => {
      if (err) {
        ...
        var token = jwt.sign({ _id: result.insertId }, CONFIG.SECRETKEY, {
          expiresIn: '24h',
        });
  ...
  if (req.body && req.body.email && req.body.password) {
    User.findOne({ email: req.body.email }, (error, user) => {
      if (error) {
        ...
        if (isPwd) {
          if (user.isActive) {
            var token = jwt.sign({ _id: user._id }, CONFIG.SECRETKEY, {
              expiresIn: '24h',
            });
            res.status(200).set('application/json').json({
              auth: true,
...
module.exports.getImage = (req, res, next) => {
  ...
  User.findById(userId, (error, response) => {
    if (error) {
      ...
      res.status(200).set('application/json').json(response.userImage);
    }
  });
};
